			 ...
			 void **metric_value)
{
	...
	if (!metric_value_type || !metric_value_len || !metric_value) {
        ...
        snprintf(mds_stat_path, PATH_MAX, "%s/%s", lmt_mds_dirname, "stats");
        rv = common_metric_lmt_get_filesystem_operations(mds_stat_path, lmtbuf, LMT_BUFLEN);
        if (rv != 0) {
