...
#define LMT_METRIC_ROUTER_MODULE_NAME "lmt_router"
#define LMT_METRIC_ROUTER_NAME        "lmt_router"

static char *
lmt_metric_router_get_name(void)
{
	return LMT_METRIC_ROUTER_NAME;
}

static int
lmt_metric_router_get_value(unsigned int *metric_value_type,
			    ...
			    void **metric_value)
{
	...
	if (!metric_value_type || !metric_value_len || !metric_value) {
        ...
        rv = common_metric_lmt_strncpy(&tmp, LMT_ROUTER_PROTOCOL_VERSION, &bufleft);
        if (rv != 0) {
        ...
        /* Get LNET Network Usage */
        rv = common_metric_lmt_get_lnet_network_usage(lmtbuf, LMT_BUFLEN);
        if (rv != 0) {
                cerebro_err_debug("problem retrieving LNET Network usage");
                goto cleanup;
        }
        rv = common_metric_lmt_strncpy(&tmp, lmtbuf, &bufleft);
        if (rv != 0) {
                cerebro_err_debug("problem copying LNET Network usage to buffer");
                goto cleanup;
        }
