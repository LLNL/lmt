...
lmt_setup_mds_ops_stmt(struct lmt_db_conn *conn)
{
        ...
        /* SUMSQUARES */
        param[5].buffer_type = MYSQL_TYPE_LONGLONG;
...
lmt_setup_ost_stmt(struct lmt_db_conn *conn)
{
        ...
        /* INODES_USED */
        param[7].buffer_type = MYSQL_TYPE_LONGLONG;
                                ...
                                char *buf)
{
        ...
        if (lmtinfo != NULL) {
                ...
                for (i = 0; i < lmtinfo->lmt_num_conn; i++) {
                        ...
                        /* Skip this insertion if we have already inserted an item
                         * for this timestamp */
                        if (mds->timestamp_id >= lmtinfo->lmt_conn[i]->timestampid) {
                                continue;
                        }
                        ...
                        *((unsigned long long *) param[2].buffer) = lmtinfo->lmt_conn[i]->timestampid;
                        ...
                        /* Update the timestamp id for this mds */
                        mds->timestamp_id = lmtinfo->lmt_conn[i]->timestampid;
                }
        }
                                ...
                                char *buf)
{
        ...
        if (lmtinfo != NULL) {
                ...
                for (i = 0; i < lmtinfo->lmt_num_conn; i++) {
                        ...
                        /* Skip this insertion if we have already inserted an item
                         * for this timestamp */
                        if (oss->timestamp_id >= lmtinfo->lmt_conn[i]->timestampid) {
                                continue;
                        }
                        ...
                        *((float *)              param[3].buffer) = itm->memory_usage;
                        ...
                        /* Update the timestamp id for this oss */
                        oss->timestamp_id = lmtinfo->lmt_conn[i]->timestampid;
                }
        }
                                ...
                                char *buf)
{
        ...
        if (lmtinfo != NULL) {
                ...
                for (i = 0; i < lmtinfo->lmt_num_conn; i++) {
                        ...
                        /* Skip this insertion if we have already inserted an item
                         * for this timestamp */
                        if (ost->timestamp_id >= lmtinfo->lmt_conn[i]->timestampid) {
                                continue;
                        }
                        ...
                        *((unsigned long long *) param[7].buffer) = itm->filestotal - itm->filesfree;
                        ...
                        /* Update the timestamp id for this ost */
                        ost->timestamp_id = lmtinfo->lmt_conn[i]->timestampid;
                }
        }
                                   ...
                                   char *buf)
{
        ...
        if (lmtinfo != NULL) {
                ...
                for (i = 0; i < lmtinfo->lmt_num_conn; i++) {
                        ...
                        /* Skip this insertion if we have already inserted an item
                         * for this timestamp */
                        if (rtr->timestamp_id >= lmtinfo->lmt_conn[i]->timestampid) {
                                continue;
                        }
                        ...
                        *((float *)              param[3].buffer) = itm->cpu_usage;
                        ...
                        /* Update the timestamp id for this router */
                        rtr->timestamp_id = lmtinfo->lmt_conn[i]->timestampid;
                }
        }
